...
const plans = [
  { id: 1, invest: 100, returns: 200, label: "Starter", icon: "ðŸŒ±", color: "#f6c90e" },
  { id: 2, invest: 200, returns: 400, label: "Growth", icon: "ðŸ“ˆ", color: "#4caf50" },
  { id: 3, invest: 500, returns: 900, label: "Pro", icon: "ðŸ’Ž", color: "#00bcd4" },
  { id: 4, invest: 1000, returns: 2100, label: "Elite", icon: "ðŸš€", color: "#ff9800" },
];
...
function TrustMarquee() {
  const text = "âœ… 1,000,000+ Trusted People â€¢ ðŸ’° Safe & Secure Investments â€¢ ðŸ† #1 Investment Platform â€¢ ðŸŒ Worldwide Trusted â€¢ ";
  return (
...
function AuthPage() {
  ...
  async function handleEmailAuth() {
    setError("");
    if (!email || !password) { setError("âš ï¸ Email aur Password bharein"); return; }
    if (mode === "register" && !name) { setError("âš ï¸ Naam bharein"); return; }
    setLoading(true);
    try {
      if (mode === "register") {
      ...
      const msg = e.code === "auth/email-already-in-use" ? "âš ï¸ Email already registered hai" :
                  e.code === "auth/wrong-password" ? "âš ï¸ Password galat hai" :
                  e.code === "auth/user-not-found" ? "âš ï¸ Email registered nahi hai" :
                  e.code === "auth/weak-password" ? "âš ï¸ Password 6+ characters ka hona chahiye" :
                  e.message;
      setError(msg);
    }
    setLoading(false);
  ...
  async function handleForgotPassword() {
    setError("");
    if (!email) { setError("âš ï¸ Pehle email likhein"); return; }
    setLoading(true);
    try {
      await sendPasswordResetEmail(auth, email);
      setError("âœ… Reset link aapki email pe bhej diya gaya hai!");
    } catch (e) {
      setError("âš ï¸ Email nahi mila ya kuch galat hua");
    }
    setLoading(false);
  }
  ...
  async function handleGoogle() {
    ...
    try {
      ...
      setError("âš ï¸ Google login failed");
      setLoading(false);
    }
  }
    ...
    <div style={{ minHeight:"100vh", background:"linear-gradient(135deg,#060d0a 0%,#0a1f0e 50%,#060d0a 100%)", display:"flex", alignItems:"center", justifyContent:"center", padding:20, position:"relative", overflow:"hidden" }}>
      ...
      <div style={{ background:"rgba(255,255,255,0.04)", borderRadius:28, padding:"36px 28px", maxWidth:380, width:"100%", border:"1px solid rgba(255,255,255,0.1)", backdropFilter:"blur(30px)", animation:"loginBoxIn 0.5s ease", display:"flex", flexDirection:"column", gap:14, position:"relative", zIndex:1 }}>
        ...
        <div style={{ textAlign:"center", marginBottom:4 }}>
          <div style={{ width:64, height:64, background:"linear-gradient(135deg,rgba(74,222,128,0.2),rgba(16,185,129,0.1))", border:"1px solid rgba(74,222,128,0.3)", borderRadius:18, display:"flex", alignItems:"center", justifyContent:"center", fontSize:28, margin:"0 auto 12px", boxShadow:"0 8px 32px rgba(74,222,128,0.15)" }}>ðŸ’°</div>
          ...
          <p style={{ color:"rgba(255,255,255,0.4)", fontSize:12, marginTop:4 }}>Smart Investing â€¢ 1 Month Returns</p>
        </div>
        ...
        {mode === "register" && (
          <input style={{ background:"rgba(255,255,255,0.06)", border:"1px solid rgba(255,255,255,0.1)", borderRadius:12, padding:"13px 16px", color:"#fff", fontSize:14, outline:"none", width:"100%" }} placeholder="ðŸ‘¤ Aapka Naam" value={name} onChange={e => setName(e.target.value)} />
        )}
        <input style={{ background:"rgba(255,255,255,0.06)", border:"1px solid rgba(255,255,255,0.1)", borderRadius:12, padding:"13px 16px", color:"#fff", fontSize:14, outline:"none", width:"100%" }} placeholder="ðŸ“§ Email" type="email" value={email} onChange={e => setEmail(e.target.value)} />
        <input style={{ background:"rgba(255,255,255,0.06)", border:"1px solid rgba(255,255,255,0.1)", borderRadius:12, padding:"13px 16px", color:"#fff", fontSize:14, outline:"none", width:"100%" }} placeholder="ðŸ”’ Password (min 6)" type="password" value={password} onChange={e => setPassword(e.target.value)} />
        ...
        {error && <div style={{ background: error.startsWith("âœ…") ? "rgba(74,222,128,0.1)" : "rgba(244,67,54,0.1)", border:`1px solid ${error.startsWith("âœ…") ? "rgba(74,222,128,0.3)" : "#f44336"}`, borderRadius:10, padding:"10px 14px", color: error.startsWith("âœ…") ? "#4ade80" : "#f87171", fontSize:13, textAlign:"center" }}>{error}</div>}
        ...
        <button style={{ padding:"14px 0", borderRadius:13, border:"none", background:"linear-gradient(135deg,#22c55e,#16a34a)", color:"#fff", fontWeight:800, fontSize:15, cursor:"pointer", opacity: loading ? 0.7 : 1, boxShadow:"0 4px 20px rgba(34,197,94,0.3)", position:"relative", overflow:"hidden" }} className="submit-btn" onClick={handleEmailAuth} disabled={loading}>
          {loading ? "â³ Please wait..." : mode === "login" ? "ðŸ”‘ Login" : "ðŸ“ Register"}
        </button>
...
export default function App() {
  ...
  async function handleChangePassword() {
    ...
    try {
      ...
      setPMsg({ type:"success", text:"âœ… Password change link aapki email pe bhej diya gaya hai!" });
    } catch (e) {
      setPMsg({ type:"error", text:"âš ï¸ Kuch galat hua, dobara try karein" });
    }
    setPLoading(false);
  }
  ...
  async function handleDeleteAccount() {
    const sure = window.confirm("âš ï¸ Pakka account delete karna hai? Ye permanently delete ho jayega.");
    if (!sure) return;
    try {
      ...
      if (e.code === "auth/requires-recent-login") {
        ...
        alert("âš ï¸ Kuch galat hua: " + e.message);
      }
    }
  }
  ...
  async function handleWithdraw() {
    if (!wName || !wAmount || !wPlan || !wInvestDate) { setWMsg({ type:"error", text:"âš ï¸ Sab fields bharein" }); return; }
    setWLoading(true); setWMsg(null);
    if (!isOneMonthPassed(wInvestDate)) { setWMsg({ type:"warn", text:"â³ Abhi 1 mahina poora nahi hua." }); setWLoading(false); return; }
    const msg = `ðŸ”” <b>WITHDRAWAL REQUEST</b>\n\nðŸ‘¤ Naam: <b>${wName}</b>\nðŸ“§ Email: <b>${user.email}</b>\nðŸ’° Amount: <b>â‚¹${wAmount}</b>\nðŸ“¦ Plan: <b>${wPlan}</b>\nðŸ“… Invest Date: <b>${wInvestDate}</b>\nðŸ• Time: ${new Date().toLocaleString("en-IN")}`;
    await sendTelegramNotification(msg);
    setWMsg({ type:"success", text:"âœ… Withdrawal request bhej di gayi!" });
    setWLoading(false);
  }
  ...
  const navItems = [
    ...
    { id:"withdraw", icon:"ðŸ’¸", label:"Withdraw" },
    ...
    <div style={{ minHeight:"100vh", background:"#060d0a", color:"#fff", position:"relative", overflow:"hidden", paddingBottom:40 }}>
      ...
      <div style={{ display:"flex", background:"rgba(0,0,0,0.6)", backdropFilter:"blur(30px)", borderBottom:"1px solid rgba(255,255,255,0.05)", position:"sticky", top:0, zIndex:100 }}>
        ...
        <button className="nav-btn" style={{ flex:1, padding:"12px 0", background:"none", border:"none", color:"#f87171", fontWeight:700, fontSize:11, cursor:"pointer", display:"flex", flexDirection:"column", alignItems:"center", gap:3, borderBottom:"2px solid transparent" }} onClick={() => signOut(auth)}>
          <span style={{ fontSize:18 }}>ðŸšª</span>Logout
        </button>
      </div>
      ...
      <div style={{ display:"flex", justifyContent:"center", padding:"12px 0 0" }}>
        <div style={{ display:"flex", alignItems:"center", gap:6, background:"rgba(255,255,255,0.04)", border:"1px solid rgba(255,255,255,0.08)", borderRadius:50, padding:"5px 14px", fontSize:11, color:"rgba(255,255,255,0.4)" }}>
          ðŸ‘‹ <span style={{ color:"#4ade80", fontWeight:600 }}>{user.displayName || user.email}</span>
        </div>
      </div>
      ...
      {page === "home" && (
        <div style={{ animation:"fadeUp 0.4s ease" }}>
          ...
          <div style={{ textAlign:"center", padding:"32px 24px 20px" }}>
            <div style={{ width:72, height:72, background:"linear-gradient(135deg,rgba(74,222,128,0.2),rgba(246,201,14,0.1))", border:"1px solid rgba(74,222,128,0.25)", borderRadius:20, display:"flex", alignItems:"center", justifyContent:"center", fontSize:32, margin:"0 auto 16px", boxShadow:"0 8px 32px rgba(74,222,128,0.12)" }}>ðŸ’°</div>
          ...
          <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr 1fr", gap:10, padding:"0 20px 24px" }}>
            {[["500+","Investors","#4ade80"],["98%","Success","#f6c90e"],["1 Mo","Returns","#60a5fa"]].map(([val,lbl,clr]) => (
              <div key={lbl} style={{ background:"rgba(255,255,255,0.03)", border:"1px solid rgba(