...
function AuthPage({ onLogin }) {
  ...
  async function handleEmailAuth() {
    ...
    try {
      ...
      const msg = e.code === "auth/email-already-in-use" ? "⚠️ Email already registered hai" :
                  ...
                  e.code === "auth/weak-password" ? "⚠️ Password kam se kam 6 characters ka hona chahiye" :
  ...
  async function handleGoogle() {
    ...
    try {
      ...
      setError("⚠️ Google login failed, dobara try karo");
    }
    setLoading(false);
  }
  ...
  return (
    <div style={authStyles.root}>
      ...
      <div style={authStyles.card}>
        ...
        <button
          style={{ ...authStyles.btn, opacity: loading ? 0.7 : 1 }}
        ...
        <button
          style={{ ...authStyles.googleBtn, opacity: loading ? 0.7 : 1 }}
...
export default function App() {
  ...
  async function handleWithdraw() {
    ...
    if (!isOneMonthPassed(wInvestDate)) {
      setWMsg({ type: "warn", text: "⏳ Abhi 1 mahina poora nahi hua. 1 mahine baad withdrawal karein." });
      setWLoading(false);
      return;
    }
    const msg =
      ...
      `✅ Status: 1 Month Complete\n` +
      `🕐 Request Time: ${new Date().toLocaleString("en-IN")}`;
    await sendTelegramNotification(msg);
    setWMsg({ type: "success", text: "✅ Withdrawal request bhej di gayi! Jaldi process hoga." });
    setWLoading(false);
  }
          ...
          <div style={wd.form}>
            ...
            {wInvestDate && (
              <div style={{ ...wd.statusBadge, background: isOneMonthPassed(wInvestDate) ? "rgba(76,175,80,0.15)" : "rgba(244,67,54,0.12)", borderColor: isOneMonthPassed(wInvestDate) ? "#4caf50" : "#f44336", color: isOneMonthPassed(wInvestDate) ? "#4caf50" : "#f44336" }}>
                {isOneMonthPassed(wInvestDate) ? "✅ 1 Month poora — Withdrawal allowed!" : "⏳ 1 Month nahi hua — Blocked"}
              </div>
            )}
      ...
      {showModal && selected && (
        <div style={styles.modalOverlay} onClick={() => setShowModal(false)}>
          <div style={styles.modal} onClick={e => e.stopPropagation()}>
            ...
            <p style={styles.modalMsg}>Payment ke liye hamare <b>Telegram Group</b> join karo.</p>
...
const authStyles = {
  ...
  divider: { textAlign: "center", color: "#555", fontSize: 12, position: "relative" },
